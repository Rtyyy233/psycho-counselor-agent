...
class EmotionalState(BaseModel):
    """情绪状态"""
    emotion: Optional[str] = Field(None,description="情绪名称，如焦虑、喜悦、悲伤")
    intensity: Optional[Literal["潜意识","弱","中","强","极强"]] = Field(None, description="情绪强度")
...
class SituationalTag(BaseModel):
    ...
    persons: List[str] = Field(default_factory=list, description="涉及人物及角色，如['母亲-权威']")
    scene_type: Optional[Literal["工作","家庭","亲密关系","社交","独处","学习","其他"]] = Field(None, description="场景类型")
    event_type: Optional[Literal["创伤","积极","重大转折","日常","冲突","成就","失落","反思","情绪抒发"]] = Field(None, description="事件类型")
    # 此处的枚举类型有待check
...
class DiaryChunk(BaseModel): # 有待check：对日记的处理模式的改进空间
    ...
    date: str = Field(None, description="日记标注日期，必须严格按照如'25.03.15'的格式标注") # pyright: ignore[reportAssignmentType]
    
    # ----- 内容层（结构化提取）-----
    emotions: EmotionalState = Field(default_factory=EmotionalState, description="主要情绪及强度") # pyright: ignore[reportArgumentType]
    cognitions: Cognition = Field(default_factory=Cognition, description="自动想法、信念、反思") # type: ignore
    behaviors: Behavior = Field(default_factory=Behavior, description="行为与后果") # type: ignore
    
    # ----- 情境层（便于检索过滤）-----
    tags: SituationalTag = Field(default_factory=SituationalTag, description="结构化标签") # type: ignore
...
class DiaryChunkbyEvent(BaseModel): #diary_chunker ver 2.0
    ...
    start_marker: str = Field(description="信息块开头的一段连续文本，约10-20字，必须是原文本的连续子串，"\
                              "必须从信息块的第一个字符开始，不得对文本进行任何修改、摘要或省略，必须原样复制")
    end_marker: str = Field(description="信息块结尾的一段连续文本，约10-20字，必须是原文本的连续子串，"\
...
async def store_diary(file_path: str) -> str :
    ...
    prompt = "你是一个有深厚心理咨询和精神医学背景的日记分析师，你需要对接收的日记片段" \
    ...
    for i in standardized_docs:
        ...
        diary_annotation_to_save.append(
            Document(
                page_content= "\n".join([i.outline or "",i.emotions.emotion or "", 
                                         i.cognitions.automatic_thought or "", 
                                         i.cognitions.belief or "", 
                                         i.cognitions.belief or "", i.cognitions.reflection or "", i.behaviors.action or "", 
                                         i.behaviors.consequence or "", i.tags.place or "", ",".join(i.tags.persons) or "" ]),
                metadata=datatable,
                id=index,
            )
        )
